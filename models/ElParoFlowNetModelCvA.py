...
class ElParoFlowNetCvA(nn.Module):
    def __init__(self):
        super().__init__()
        self.max_flow = 8
...
if __name__ == '__main__':
    ...
    h = 1
    w = 15
    mv = torch.zeros(5, 5, h, w)
    # for yi in range(-mf, mf + 1):
    #     yix = max(0, yi)
    #     yin = min(yi, 0)
    for xi in range(-mf, mf + 1):
        xix = max(0, xi)  # 0 to 8
        xin = min(xi, 0)  # -8 to 0

        #print(yix,yin,' ', xix, xin)
        print('{}:{}\t{}:{}'.format(xix, w + xin, -xin, w - xix))