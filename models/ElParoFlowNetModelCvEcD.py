...
class ElParoFlowNetCvEcD(nn.Module):
    def __init__(self):
        super().__init__()
        self.max_flow = 8
        ...
        self.flow_net = nn.Sequential(
            nn.Conv3d(self.flow_size, self.cv_size // 4, (self.flow_size, 1, 1)),
            nn.BatchNorm3d(self.cv_size // 4),
            nn.LeakyReLU(),
            nn.Conv3d(self.cv_size // 4, self.cv_size // 4, 1),
            nn.BatchNorm3d(self.cv_size // 4),
            nn.LeakyReLU(),
            ...
            nn.BatchNorm3d(self.cv_size // 4),
            nn.LeakyReLU(),
            nn.Conv3d(self.cv_size // 4, self.cv_size // 2, (1, 3, 3)),
            nn.BatchNorm3d(self.cv_size // 2),
            nn.LeakyReLU(),
            nn.Conv3d(self.cv_size // 2, self.cv_size // 4, 1),
            nn.BatchNorm3d(self.cv_size // 4),
