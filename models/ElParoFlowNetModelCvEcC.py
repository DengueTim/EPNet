...
class ElParoFlowNetCvEcC(nn.Module):
    def __init__(self):
        super().__init__()
        self.max_flow = 8
        ...
        self.flow_net = nn.Sequential(
            nn.ReLU(),
            nn.Conv3d(self.flow_size, self.cv_size // 2, (self.flow_size, 1, 1)),
            ...
            nn.LeakyReLU(),
            nn.Conv3d(self.cv_size // 8, self.cv_size // 2, (1,3,3), padding=(0,1,1)),
            nn.BatchNorm3d(self.cv_size // 2),
            nn.LeakyReLU(),
            nn.Conv3d(self.cv_size // 2, self.cv_size // 4, 1),
            nn.BatchNorm3d(self.cv_size // 4),
    ...
    def forward(self, patch_a, patch_b):
        ...
        for yi in range(-max_flow, max_flow + 1):
            yix = max(0,yi)
            yin = min(yi,0)
            for xi in range(-max_flow, max_flow + 1):
                xix = max(0,xi) # 0 to 8
                xin = min(xi,0) # -8 to 0

                features_a_cropped = features_a[:, :,  yix:h + yin, xix:w + xin]
                features_b_cropped = features_b[:, :, -yin:h - yix, -xin:w - xix]
                features_product = features_a_cropped * features_b_cropped
                mv[:, yi + max_flow, xi + max_flow, yix:h + yin, xix:w + xin] = torch.sum(features_product, dim=1)
